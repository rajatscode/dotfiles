# 40-agents.bashrc - AI Agent Workflow Integration
...
if [ "${AGENT_AUTO_SYNC:-false}" = "true" ]; then
    # Add to PROMPT_COMMAND to sync before each prompt
    export PROMPT_COMMAND="context-sync --once 2>/dev/null; ${PROMPT_COMMAND}"
fi
...
_agent_sessions() {
    local sessions=$(agent list 2>/dev/null | awk '/^[a-zA-Z]/ {print $1}' 2>/dev/null)
    COMPREPLY=($(compgen -W "${sessions}" -- "${COMP_WORDS[COMP_CWORD]}"))
}

complete -F _agent_sessions asw acd actx async aclose ashare alock aunlock
