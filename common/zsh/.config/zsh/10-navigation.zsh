# 10-navigation.zsh - Navigation System
...
# Zsh completion for fal
_fal_completion() {
    local -a stored_aliases
    stored_aliases=($(ls -1 "$ALIAS_SYMLINK_DIR" 2>/dev/null))
    _describe 'alias' stored_aliases
}
compdef _fal_completion fal

## `xal` - removes aliases
## if no arguments are given, clears all aliases
xal() {
    if [ "$#" -eq 0 ]; then
        read "yn?You're about to delete all symlink aliases. Are you sure? [y/N] "
        case $yn in
...
deldir() {
    ...
    if [ $(ls -a | wc -l) -le 2 ]; then
        ...
        read "yn?You're about to delete non-empty dir ${PWD##*/}. Are you sure? [y/N] "
        case $yn in
            [Yy]* ) confirmdel=1 ;;
            [Nn]* ) confirmdel=0 ;;
            * )
